...
function CheckerPage() {
  ...
  const downloadPDF = async () => {
    ...
    doc.text(new Date(report.fetchedAt).toLocaleString(), 40, 68);
    y = 110;
    doc.setTextColor(20); doc.setFontSize(12);
    doc.text(`URL: ${report.url}`, 40, y); y += 16;
    doc.text(`Final: ${report.finalUrl}  •  Status: ${report.status ?? "n/a"}`, 40, y); y += 24;
